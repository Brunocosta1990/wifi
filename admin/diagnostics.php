...
$items=[]; $cfg=app_config(); $tz=$cfg['timezone'] ?? 'America/Sao_Paulo';
...
try{ $pdo=db(); $items[]=['Banco conexão','OK',$pdo->query('SELECT VERSION()')->fetchColumn()]; $charset=$pdo->query("SELECT CONCAT(@@character_set_connection, ' / ', @@collation_connection)")->fetchColumn(); $items[]=['Charset conexão',str_contains((string)$charset,'utf8mb4_unicode_ci')?'OK':'Atenção',(string)$charset]; }catch(Throwable $e){ $items[]=['Banco conexão','Erro',$e->getMessage()]; }
...
try{
    $hb=db()->query('SELECT * FROM cron_heartbeat WHERE id=1')->fetch();
    $lastFinished=$hb['last_finished_at']??null;
    $age=$lastFinished ? max(0, time() - strtotime($lastFinished.' UTC')) : null;
    $items[]=['Cron última execução',$hb?'OK':'Atenção',$lastFinished ? local_from_utc($lastFinished,$tz,'d/m/Y H:i:s').' ('.$age.'s atrás)' : 'sem registro'];
    $items[]=['Cron frequência',$age===null?'Atenção':($age<=90?'OK':'Atenção'),$age===null?'sem heartbeat':'ideal: até 90s; atual: '.$age.'s'];
    $items[]=['Cron último erro',empty($hb['last_error'])?'OK':'Erro',$hb['last_error']??''];
    $summary=json_decode((string)($hb['last_summary_json']??''),true);
    if(is_array($summary) && isset($summary['max_schedule_delay_seconds'])) $items[]=['Maior atraso de agendamento no último ciclo',(int)$summary['max_schedule_delay_seconds']<=90?'OK':'Atenção',(string)$summary['max_schedule_delay_seconds'].'s'];
}catch(Throwable $e){ $items[]=['Cron heartbeat','Atenção',$e->getMessage()]; }
admin_header('Diagnóstico do sistema','diagnostics'); ?>
<div class="toolbar"><a class="btn btn-primary" href="<?= e(app_url('admin/logs.php')) ?>">Abrir logs</a></div><section class="card panel"><h2>Ambiente, Banco, Web Push, PWA e Cron</h2><p class="muted">Horários exibidos em <?= e($tz) ?>. O Cron em hospedagem compartilhada normalmente roda a cada minuto; atraso acima de 90 segundos indica frequência incorreta, fila lenta ou limitação do provedor.</p><div class="table-wrap"><table class="data-table"><tr><th>Item</th><th>Status</th><th>Detalhe</th></tr><?php foreach($items as $i) echo rowdiag($i[0],$i[1],$i[2]); ?></table></div></section>
<?php admin_footer(); ?>
